...
namespace SoftwareSuite.Controllers.AdminServices
{
    public class AdminServiceController : ApiController
    {
        ...
        public HttpResponseMessage logoutUser(string UserLoginName)
        {
            try
            {
                var dbHandler = new dbHandler();
                ...
                var dt = dbHandler.ReturnDataWithStoredProcedureTable("SP_Set_UserLogout", param);
                HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.OK, dt);
                return response;
            }
            catch (Exception ex)
            {
                dbHandler.SaveErorr("SP_Set_UserLogout", 0, ex.Message);
                throw ex;
            }
        }
